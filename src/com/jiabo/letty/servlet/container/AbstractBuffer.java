...
public abstract class AbstractBuffer implements Buffer {
	...
	private final static boolean __boundsChecking = Boolean
			.getBoolean("org.eclipse.jetty.io.AbstractBuffer.boundsChecking");

	protected final static String __IMMUTABLE = "IMMUTABLE",
	 ...
	 * 
	 * @param access
	 *            0==IMMUTABLE, 1==READONLY, 2==READWRITE
	 */
	...
	public ByteArrayBuffer duplicate(int access) {
		...
		if (this instanceof Buffer.CaseInsensitve
				|| b instanceof Buffer.CaseInsensitve)
			return new ByteArrayBuffer.CaseInsensitive(asArray(), 0, length(),
					access);
		else
	...
	public Buffer peek(int index, int length) {
		if (_view == null) {
			_view = new View(this, -1, index, index + length,
	...
	public int poke(int index, Buffer src) {
		...
		if (src_array != null && dst_array != null)
			System.arraycopy(src_array, src.getIndex(), dst_array, index,
					length);
	...
	public void setGetIndex(int getIndex) {
		/*
		 * bounds checking if (isImmutable()) throw new
		 * IllegalStateException(__IMMUTABLE); if (getIndex < 0) throw new
		 * IllegalArgumentException("getIndex<0: " + getIndex + "<0"); if
		 * (getIndex > putIndex()) throw new
		 * IllegalArgumentException("getIndex>putIndex: " + getIndex + ">" +
		 * putIndex());
		 */
		_get = getIndex;
		_hash = 0;
	...
	public void setMarkIndex(int index) {
		/*
		 * if (index>=0 && isImmutable()) throw new
		 * IllegalStateException(__IMMUTABLE);
		 */
		_mark = index;
	}
	...
	public void setPutIndex(int putIndex) {
		/*
		 * bounds checking if (isImmutable()) throw new
		 * IllegalStateException(__IMMUTABLE); if (putIndex > capacity()) throw
		 * new IllegalArgumentException("putIndex>capacity: " + putIndex + ">" +
		 * capacity()); if (getIndex() > putIndex) throw new
		 * IllegalArgumentException("getIndex>putIndex: " + getIndex() + ">" +
		 * putIndex);
		 */
		_put = putIndex;
	...
	public void writeTo(OutputStream out) throws IOException {
		...
		if (array != null) {
			...
			int offset = _get;
			while (len > 0) {
				int l = peek(offset, buf, 0, len > buf.length ? buf.length
						: len);
				out.write(buf, 0, l);
				offset += l;
				len -= l;
