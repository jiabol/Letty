...
import com.jiabo.letty.servlet.container.BufferCache.CachedBuffer;
...
public class HttpParser {
	...
	private Buffer _buffer; // The current buffer in use (either _header or
							// _content)
	private CachedBuffer _cached;
	private final View.CaseInsensitive _tok0; // Saved token: header name,
												// request method or response
												// version
	private final View.CaseInsensitive _tok1; // Saved token: header value,
												// request URI or response code
	 ...
	 * -----
	 */
	protected final View _contentView = new View(); // View of the content in
													// the buffer for {@link
													// Input}
	...
	public boolean isMoreInBuffer() throws IOException {
		return (_header != null && _header.hasContent() || _body != null
				&& _body.hasContent());
	}
	 ...
	 */
	/**
	 * Parse until {@link #STATE_END END} state. If the parser is already in the
	 * END state, then it is {@link #reset reset} and re-parsed.
	 * 
	 ...
	 */
	/**
	 * Parse until END state. This method will parse any remaining content in
	 * the current buffer as long as there is no unconsumed content. It does not
	 * care about the {@link #getState current state} of the parser.
	 * 
	 * @see #parse
	 * @see #parseNext
	 */
			...
			while (_state < STATE_END && length-- > 0) {
				...
				case STATE_FIELD0:
					if (ch == HttpTokens.SPACE) {
						_tok0.update(_buffer.markIndex(),
								_buffer.getIndex() - 1);
						_responseStatus = HttpVersions.CACHE.get(_tok0) == null ? -1
								: 0;
						_state = STATE_SPACE1;
						continue;
				...
				case STATE_STATUS:
					if (ch == HttpTokens.SPACE) {
						_tok1.update(_buffer.markIndex(),
								_buffer.getIndex() - 1);
						_state = STATE_SPACE2;
						continue;
					} else if (ch >= '0' && ch <= '9') {
						...
						_handler.startResponse(HttpMethods.CACHE.lookup(_tok0),
					...
					if (ch > HttpTokens.SPACE || ch < 0) {
						...
						if (_responseStatus > 0) {
							...
							_handler.startRequest(
									HttpMethods.CACHE.lookup(_tok0), _tok1,
									null);
							_persistent = false;
							_state = STATE_SEEKING_EOF;
							_handler.headerComplete();
					...
					if (ch == HttpTokens.CARRIAGE_RETURN
						...
						if (_responseStatus > 0)
							...
							_handler.startRequest(
									HttpMethods.CACHE.lookup(_tok0),
									_tok1,
									version = HttpVersions.CACHE.lookup(_buffer
											.sliceFromMark()));
						_eol = ch;
				...
				case STATE_HEADER:
					...
					default: {
						...
						if (_cached != null || _tok0.length() > 0
								|| _tok1.length() > 0
								|| _multiLineValue != null) {
							Buffer header = _cached != null ? _cached
									: HttpHeaders.CACHE.lookup(_tok0);
							_cached = null;
							Buffer value = _multiLineValue == null ? _tok1
									: new ByteArrayBuffer(_multiLineValue);
							...
							if (ho >= 0) {
								...
								case HttpHeaders.TRANSFER_ENCODING_ORDINAL:
									value = HttpHeaderValues.CACHE
											.lookup(value);
									vo = HttpHeaderValues.CACHE
											.getOrdinal(value);
									if (HttpHeaderValues.CHUNKED_ORDINAL == vo)
										_contentLength = HttpTokens.CHUNKED_CONTENT;
									else {
										String c = value
												.toString(StringUtil.__ISO_8859_1);
										if (c.endsWith(HttpHeaderValues.CHUNKED))
											_contentLength = HttpTokens.CHUNKED_CONTENT;

										else if (c
												.indexOf(HttpHeaderValues.CHUNKED) >= 0)
											throw new HttpException(400, null);
									}
									break;

								case HttpHeaders.CONNECTION_ORDINAL:
									switch (HttpHeaderValues.CACHE
											.getOrdinal(value)) {
						...
						if (ch == HttpTokens.CARRIAGE_RETURN
							...
							switch (_contentLength > Integer.MAX_VALUE ? Integer.MAX_VALUE
									: (int) _contentLength) {
							...
							case HttpTokens.NO_CONTENT:
								_handler.headerComplete();
								_state = _persistent
										|| (_responseStatus >= 100 && _responseStatus < 200) ? STATE_END
										: STATE_SEEKING_EOF;
								_handler.messageComplete(_contentPosition);
								return 1;
							...
							// try cached name!
							if (array != null) {
								_cached = HttpHeaders.CACHE.getBest(array,
										_buffer.markIndex(), length + 1);

								if (_cached != null) {
									_length = _cached.length();
									_buffer.setGetIndex(_buffer.markIndex()
											+ _length);
									length = _buffer.length();
								}
							}
						...
						if (_length > 0) {
							if (_tok1.length() == 0)
								...
								_tok1.update(_buffer.markIndex(),
										_buffer.markIndex() + _length);
								_multiLineValue += " "
										+ _tok1.toString(StringUtil.__ISO_8859_1);
							}
						}
						_eol = ch;
						_state = STATE_HEADER;
	...
	protected int fill() throws IOException {
		...
		if (_state > STATE_END && _buffer == _header && _header != null
				&& !_header.hasContent() && _body != null && _body.hasContent()) {
			_buffer = _body;
		...
		if (_buffer == _header
				&& _state > STATE_END
				&& _header.length() == 0
				&& (_forceContentBuffer || (_contentLength - _contentPosition) > _header
						.capacity()) && (_body != null || _buffers != null)) {
			if (_body == null)
	...
	public void reset() {
		...
		_state = _persistent ? STATE_START
				: (_endp.isInputShutdown() ? STATE_END : STATE_SEEKING_EOF);
		_contentLength = HttpTokens.UNKNOWN_CONTENT;
		_contentPosition = 0;
		_length = 0;
		_responseStatus = 0;
		...
		// Consume LF if CRLF
		if (_eol == HttpTokens.CARRIAGE_RETURN && _buffer != null
				&& _buffer.hasContent()
				&& _buffer.peek() == HttpTokens.LINE_FEED)
			_eol = _buffer.get();
		...
		if (_body != null && _body.hasContent()) {
			...
			// This is probably a pipelined header of the next request, so we
			// need to
	...
	public void returnBuffers() {
		...
		if (_header != null && !_header.hasContent()
				&& _header.markIndex() == -1 && _buffers != null) {
			if (_buffer == _header)
				_buffer = null;
	...
	public String toString(Buffer buf) {
		return "state=" + _state + " length=" + _length + " buf="
				+ buf.hashCode();
	}
	...
	public String toString() {
		return String.format("%s{s=%d,l=%d,c=%d}", getClass().getSimpleName(),
				_state, _length, _contentLength);
	}
	...
	/**
	 * @param force
	 *            True if a new buffer will be forced to be used for content and
	 *            the header buffer will not be used.
	 */
	...
	public Buffer blockForContent(long maxIdleTime) throws IOException {
		...
		try {
			...
			// timeout)
			while (_contentView.length() == 0
					&& !(isState(HttpParser.STATE_END) || isState(HttpParser.STATE_SEEKING_EOF))
					&& _endp != null && _endp.isOpen()) {
				if (!_endp.isBlocking()) {
					if (parseNext() > 0)
						continue;
	...
	public int available() throws IOException {
		...
		if (_endp.isBlocking()) {
			if (_state > 0 && _endp instanceof StreamEndPoint)
				return ((StreamEndPoint) _endp).getInputStream().available() > 0 ? 1
						: 0;
	...
	public static abstract class EventHandler {
		 ...
		 * parsed
		 */
		public abstract void startRequest(Buffer method, Buffer url,
				Buffer version) throws IOException;

		/**
		 * This is the method called by parser when the HTTP request line is
		 * parsed
		 */
		public abstract void startResponse(Buffer version, int status,
