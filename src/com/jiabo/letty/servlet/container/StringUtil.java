...
public class StringUtil {
	...
	public static boolean isUTF8(String charset) {
		return __UTF8.equalsIgnoreCase(charset)
				|| __UTF8Alt.equalsIgnoreCase(charset);
	}
	...
	public static String sidBytesToString(byte[] sidBytes) {
		...
		for (int i = 0; i < subAuthorityCount; ++i) {
			...
			sidString.append('-').append(
					Long.parseLong(tmpBuilder.toString(), 16));
		}
	...
	public static byte[] sidStringToBytes(String sidString) {
		...
		for (int i = 0; i < hexStr.length(); i = i + 2) {
			sidBytes[byteCount++] = (byte) Integer.parseInt(
					hexStr.substring(i, i + 2), 16);
		}
